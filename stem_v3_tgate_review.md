...
**리뷰 역할**: @domain @arch @engineer @reviewer
...
| 관점 | 분석 |
|------|------|
| **@domain** | Context 프레임은 clean 과거 관측. 노이즈 레벨 `t`와 무관. t-conditioning은 의미론적으로 부적절 |
| **@arch** | `x_embedder`와 `context_embedder`는 **weight 비공유** (line 548 vs 554). 독립 모듈이므로 한쪽만 변경 가능 |
| **@engineer** | `context_embedder`는 for-loop 내에서 프레임별 호출 (line 1477-1480). t_emb 전달은 간단하나 불필요한 연산 추가 |
| **@reviewer** | 단일 변수 실험 원칙: t-gated의 효과를 순수 측정하려면 target side만 변경이 올바름 |
...
| 관점 | 분석 |
|------|------|
| **@arch** | `TimestepEmbedder(hidden_size=512)` → 출력 `(B, 512)`. 표준적 |
| **@engineer** | Per-stage projection: `Linear(512, 2*c_out)`. Stage별 파라미터 합산 ~985K. 전체 모델 대비 무시 가능 |

#### @reviewer — raw t_emb vs cond 선택 검증
