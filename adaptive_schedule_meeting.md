...
> 일시: 2026-04-24
> 참석: @reviewer (학술 검토), @domain (태양광 예보/확산모델)
...
### @reviewer: 왜 pixel-wise schedule은 원리적으로 SSIM을 파괴하는가
...
@domain 의견: 개념적으로 가장 안전하다. 다만 loss weighting은 "모델에게 특정 영역을 더 열심히 학습하라"는 것이지, "특정 영역의 신호를 더 보존하라"는 것은 아니다. 학습이 수렴한 후에는 고분산 영역의 예측이 개선되겠지만, 추론 시 noise schedule이 동일하므로 신호 보존 효과는 없다.
...
@reviewer 의견: 이론적으로 가장 타당하다. 모델이 variance 정보를 어떻게 활용할지를 학습하게 두는 것이 하드코딩된 schedule 변경보다 유연하다. 다만 prototyping에서 효과를 보려면 학습 epoch이 충분해야 한다.
...
@domain 의견: per-channel SNR은 **loss 가중치**이고, 대역별 schedule은 **noise 주입량** 자체를 바꾸는 것이다. 이 둘은 다른 메커니즘이다. LL 대역의 신호를 더 보존하면 denoising 시 대규모 공간 구조(일사량의 공간 패턴)가 더 잘 유지될 수 있다.

@reviewer 의견: 이중 보정 리스크(13B v1에서 겪은 문제)를 피하려면, 이 방향을 채택할 경우 per-channel SNR weighting은 비활성화하고 ablation 비교해야 한다.
...
@reviewer 의견: 가장 보수적이고 안전한 접근. SSIM을 파괴하지 않을 가능성이 높다. 다만 효과 크기가 미미할 수 있다.
