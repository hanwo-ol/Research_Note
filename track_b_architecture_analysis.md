...
| # | 병목 | 관련 모듈 | 증거 | 관점 합의 |
|---|---|---|---|---|
| **L1** | **공간 해상도 부족 (P=32 → 256 tokens)** | Stem + Backbone | patch=16에서 direction_corr +91% (0093) | @arch + @domain |
| **L2** | **Motion 학습 경로 제한 (SPADE affine만, warp 없음)** | Dynamic Decoder | σ_resid 병목 Pearson r=0.998 (0090 §12.8) | @arch + @domain |
| **L3** | **Skip connection 부재 (U-Net advantage 미활용)** | Stem → Decoder | Deep Skip 실패(D-14~16)지만 t-gate 미적용 | @arch + @reviewer |

### 5.2 각 관점의 독자적 지적

| 관점 | 독자 지적 | 모듈 |
|---|---|---|
| **@engineer** | `inspect.signature` 런타임 호출 → torch.compile graph break | Backbone |
| **@reviewer** | Normalization 제거의 정량적 근거 부재 | Stem + SPADE |
| **@reviewer** | Dynamic Decoder 단독 기여 미분리 (D-7 vs D-60 tie) | Dynamic |
| **@domain** | 야간(GT≈0) 영역 과잉 처리, 계절별 차이 미반영 | 전체 |
